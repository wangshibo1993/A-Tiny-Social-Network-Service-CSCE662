...
class IClient
{
    ...
    private:
        ...
        void displaySuccessfulReply(const std::string& comm, const IReply& reply) ;
...
void IClient::displaySuccessfulReply(const std::string& comm, const IReply& reply){
...
void IClient::displayCommandReply(const std::string& comm, const IReply& reply)  
{
	if (reply.grpc_status.ok()) {
		displaySuccessfulReply(comm, reply);
	} else {
		//std::cout << "grpc failed: " << reply.grpc_status.error_message() << std::endl;
		int ret = connectTo();//reconnect to next available server
        if (ret < 0) {
            std::cout << "connection failed: " << ret << std::endl;
            exit(1);
        }
        std::string newcomm = reply.input;
        IReply newreply = processCommand(newcomm);
        displaySuccessfulReply(newcomm, newreply);
